// ══════════════════════════════════════════════════════════════
// useQueueStatus — [HMI v2] 작업 큐 상태. 두 소스를 합친다:
...
// 경로예약 항목의 색(variant)도 로봇 고정색이 아니라 현재 배정된 task 에서 유도(미배정이면
// 중립 기본값 waste 폴백).
// ══════════════════════════════════════════════════════════════
...
export function useQueueStatus(): QueueItem[] {
  ...
  for (const carterId of CARTER_IDS) {
    ...
    const task = snapshots[carterId]?.task;
    const variant = task ? TASK_META[task].variant : "waste";
