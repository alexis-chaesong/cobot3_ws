...
class RobotBridgeManager:
    ...
    def start_bridge(
        ...
        #   못 받는다(carter1 은 미션이 빨리 움직여 금방 갱신되지만 carter2 는 초반에 오래 정지 →
        #   지도에 안 보임). TF(map→base_link) 는 이 임계값과 무관하게 계속 흐르므로, 로봇별
        #   전용 버퍼에 /carterN/tf(+tf_static) 를 직접 먹여 주기적으로 조회 → 정지 상태에서도
        #   즉시 표시. 프레임 id 가 로봇마다 동일(map/odom/base_link, 접두 없음)해서 버퍼를
        #   공유하면 충돌하므로 로봇별로 분리한다.
        for carter_id in CARTER_IDS:
            buf = tf2_ros.Buffer()
        ...
        #   현재는 carter1(소독)만 YOLO 카메라가 있지만, 구독 자체는 두 로봇 모두 걸어둔다
        #   (carter2 에 YOLO 가 추가되는 순간 코드 변경 없이 바로 스트림 가능).
    ...
    def _process_state_callback(self, msg, robot_id: str) -> None:
        ...
        self._track_task_history(robot_id, payload)
        self._dispatch(
            {
                "type": "PROCESS_STATE",
                "robotId": robot_id,
